...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        last_hidden_joined = torch.cat([last_hidden[0], last_hidden[1]], 1)
        last_cell_joined = torch.cat([last_cell[0], last_cell[1]], 1)

        init_decoder_hidden = self.h_projection(last_hidden_joined)
        init_decoder_cell = self.c_projection(last_cell_joined)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y_matrices = torch.split(Y, 1)
        for Y_t in Y_matrices:
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        a_t = torch.bmm(unsqueezed_alpha_t, enc_hiddens)
